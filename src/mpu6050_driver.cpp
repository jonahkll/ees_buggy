...
MPU6050::MPU6050(int8_t addr, bool run_update_thread) {
    ...
    _first_run = 1; // Variable for whether to set gyro angle to acceleration
                    // angle in compFilter
    calc_yaw = false;
    ...
    if (status < 0) {
        std::cout
            << "ERR (MPU6050.cpp:MPU6050()): Could not get I2C bus with "
            << addr
    ...
    wiringPiI2CWriteReg8(
        f_dev, 0x6b,
        0b00000000); // Take MPU6050 out of sleep mode - see Register Map

    wiringPiI2CWriteReg8(f_dev, 0x1a,
                         0b00000011); // Set DLPF (low pass filter) to 44Hz (so
                                      // no noise above 44Hz will pass through)

    wiringPiI2CWriteReg8(
        f_dev, 0x19,
        0b00000100); // Set sample rate divider (to 200Hz) - see Register Map

    wiringPiI2CWriteReg8(
        f_dev, 0x1b,
        GYRO_CONFIG); // Configure gyroscope settings - see Register Map (see
                      // MPU6050.h for the GYRO_CONFIG parameter)

    wiringPiI2CWriteReg8(
        f_dev, 0x1c,
        ACCEL_CONFIG); // Configure accelerometer settings - see Register Map
                       // (see MPU6050.h for the GYRO_CONFIG parameter)

    // Set offsets to zero
    wiringPiI2CWriteReg8(f_dev, 0x06, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x07, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x08, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x09, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x0A, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x0B, 0b00000000),
        wiringPiI2CWriteReg8(f_dev, 0x00, 0b10000001),
        wiringPiI2CWriteReg8(f_dev, 0x01, 0b00000001),
        wiringPiI2CWriteReg8(f_dev, 0x02, 0b10000001);

    if (run_update_thread) {
        std::thread(&MPU6050::_update, this)
            .detach(); // Create a seperate thread, for the update routine to
                       // run in the background, and detach it, allowing the
                       // program to continue
    }
}
...
void MPU6050::getGyroRaw(float *roll, float *pitch, float *yaw) {
    int16_t X = wiringPiI2CReadReg8(f_dev, 0x43) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x44); // Read X registers
    int16_t Y = wiringPiI2CReadReg8(f_dev, 0x45) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x46); // Read Y registers
    int16_t Z = wiringPiI2CReadReg8(f_dev, 0x47) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x48); // Read Z registers
...
void MPU6050::getAccelRaw(float *x, float *y, float *z) {
    int16_t X = wiringPiI2CReadReg8(f_dev, 0x3b) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x3c); // Read X registers
    int16_t Y = wiringPiI2CReadReg8(f_dev, 0x3d) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x3e); // Read Y registers
    int16_t Z = wiringPiI2CReadReg8(f_dev, 0x3f) << 8 |
                wiringPiI2CReadReg8(f_dev, 0x40); // Read Z registers
    *x = (float)X;
    *y = (float)Y;
    *z = (float)Z;
}
...
void MPU6050::getOffsets(float *ax_off, float *ay_off, float *az_off,
...
int MPU6050::getAngle(int axis, float *result) {
    if (axis >= 0 && axis <= 2) { // Check that the axis is in the valid range
        *result = _angle[axis];   // Get the result
        ...
        std::cout
            << "ERR (MPU6050.cpp:getAngle()): 'axis' must be between 0 and 2 "
